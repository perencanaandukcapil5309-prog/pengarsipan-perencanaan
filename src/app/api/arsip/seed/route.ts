...
export async function POST() {
  try {
    ...
    // Add UUIDs to each row
    const rowsWithIds = SAMPLE_DATA.map((row) => ({
      ...row,
    ...
    const insertedCount = rowsWithIds.length;

    await logActivity(
      "IMPORT",
      `${insertedCount} dokumen seed`,
      "Data contoh Bagian Perencanaan Dukcapil Kab. Ngada",
    );

    return NextResponse.json({
      message: `${insertedCount} dokumen contoh berhasil ditambahkan.`,
      count: insertedCount,
    });
  } catch (error) {
